...
def test_and_val(pred_y, y, mode='val', epoch=0):
    ...
    auc1 = roc_auc_score(test_y, preds)
    pr, re, _ = precision_recall_curve(test_y, preds)
    aupr = auc(re, pr)
    #print("aupr......",aupr)
    
    # 计算正确预测的样本数
    correct_predictions = np.sum(test_y == preds)  # 预测正确的样本数

    # 计算准确率
    accuracy = correct_predictions / len(test_y)  # 准确率
